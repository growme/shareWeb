...
function cashMoney() {
    var checkdata = {
        wechat      :   $("#payAccount").val() ,
        wechatname  :   $("#accountName").val(),
        captcha     :   $("#captcha").val(),
    ...
    //验证支付宝账户
    if(!isWecahtAcc(checkdata.wechat)){
        M._alert("请输入正确的微信账号");
        return false;
    }

    //验证支付宝姓名
    if(!isName(checkdata.wechatname)){
        M._alert("请输入正确的微信姓名");
        return false;
    };
    ...
    $.ajax({
        ...
        cache: false,
        data: {
        	alipay:checkdata.wechat,
        	alipayname:checkdata.wechatname,
