...
function PCAS() {
	var h = [], b = arguments.length, i, w = 0, v, u = PCAS, g = this, p = function(
			s) {
		var n, O, i, s = s || 0, k, z, F = "u.c,u.P[g['i0']-1],u.I[g['i0']-1][g['i1']-1]"
				.split(",");
		for (k = s; k < b; k++) {
			if (w && s == k) {
				k++
			...
			for (i = 0; i < b - 1; i++) {
				g["i" + i] = g["l" + i].selectedIndex;
				if (g["n" + i].length == 0) {
					g["i" + i]++
				}
			}
			g["l" + k].length = g["n" + k] ? 1 : 0;
			if (k > 0 && g["l" + (k - 1)].length == 0 || g["i" + (k - 1)] == 0
					&& g["n" + (k - 1)].length > 0) {
				continue
			}
			z = eval(F[k]);
			for (i = 0; i < z.length; i++) {
				...
				g["l" + k].options.add(new Option(n, O));
				if (g["f" + k] == O) {
					g["l" + k][g["n" + k] ? (i + 1) : i].selected = true
				}
			}
		}
		w = 1
	...
	if (!u.S) {
		(u.S = function(o, e, q) {
			document.all ? o.attachEvent("on" + e, q) : o.addEventListener(e,
					q, false)
		})
				(
						window.document,
						"keydown",
						function(e) {
							if ((e || window.event).keyCode == 113) {
								alert("PCASClass "
										+ u.e
										+ "\nClass Of Province City Area Selector(JS)\nDataVersion "
										+ u.d
										+ "\nhttp://www.popub.net/script/PCASClass.html\nYongxiang Cui(333) zhadan007@21cn.com")
							}
						});
		(function() {
			u.e = "3.0.140215";
			var j, O, y, T, i, s, a, N, t, o = String.fromCharCode(26);
			a = u.Data.split("#");
			u.c = [], u.P = [], u.I = [];
	...
	for (i = 0; i < b; i++) {
		h[i] = [ "l" + i, "f" + i, "n" + i, "m" + i, "x" + i ];
		v = /([^=,]*)\=?([^,]*),?(.*)/.exec(arguments[i].toString());
		g[h[i][4]] = 0;
		g[h[i][0]] = document.getElementById(v[1])
				|| document.getElementsByName(v[1])[0];
		g[h[i][1]] = g[h[i][3]] = v[2];
		g[h[i][2]] = v[3];
		g[h[i][0]].length = 0;
		if (g[h[i][2]]) {
			g[h[i][0]].options.add(new Option(g[h[i][2]], ""))
		}
		if (i < b - 1) {
			u.S(g["l" + i], "change", (function(i) {
				return function() {
					p(i)
				}
			})(i))
