...
public class WeixinpayUtil {
	...
	public static String createDocumentForEnterprisesPayment(EnterprisesPayment enterprisesPayment) {
		...
		result.append("<appid>" + enterprisesPayment.getMch_appid() + "</appid>");
		...
		result.append("<body>" + enterprisesPayment.getSign() + "</body>");
		result.append("<notify_url>" + enterprisesPayment.getSign() + "</notify_url>");
		result.append("<trade_type>" + "APP" + "</trade_type>");
		result.append("</xml>");
	...
	public static String getOrderId() {
		...
		return "a" + String.format("%018d", hashCodeV) + new Date().getTime();
	}
